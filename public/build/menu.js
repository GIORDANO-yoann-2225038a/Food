document.addEventListener('DOMContentLoaded', () => {
    ...
    function afficherMenu(recipes) {
        ...
        recipes.forEach((recipe, index) => {
            ...
            card.className = 'bg-gray-100 shadow-2xl rounded-2xl p-6 flex flex-col items-center w-48 h-48 md:w-64 md:h-80 lg:w-80 lg:h-96 transition-transform transform hover:scale-105';

            card.innerHTML = `
                <h3 class="text-lg font-semibold text-gray-700 mb-2">${day}</h3>
                <img src="${imagePath}" alt="${recipe.title}" class="w-full h-40 object-cover rounded-md mb-3">
